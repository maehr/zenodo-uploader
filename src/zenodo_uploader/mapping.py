...
# Convenience ISO 639-1 -> 639-3 lookups. Zenodo wants 639-3; any code that is
# not listed here is passed through unchanged.
...
def default_description(record: WorkRecord) -> str:
    ...
    Examples:
        ...
        '<p>T. P, 2024.</p><p>Available at <a href="https://doi.org/10.1/x">\
doi.org/10.1/x</a> and <a href="https://example.org/x">example.org/x</a>.</p>'
    """
    ...
    if record.landing_url:
        ...
        links += f' and <a href="{record.landing_url}">{label}</a>'
    return first + f"<p>Available at {links}.</p>"
...
def work_to_zenodo(
    ...
    *,
    ...
    Community membership is not part of the metadata. Zenodo stores the legacy
    ``communities`` deposit field but ignores it, so a record joins a community
    through a review request instead. See :func:`zenodo_uploader.batch.mirror_entry`.
